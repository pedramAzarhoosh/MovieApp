...
class MovieRepository {
    ...
    private val mutableUpcomingMovies = MutableLiveData<List<Movie>>()
    ...
    public fun getUpComingMovies():MutableLiveData<List<Movie>>{
        ...
        val call: Call<MovieResult> = movieApiService.getUpcomingMovies(application.applicationContext.getString(R.string.api_key))
        ...
        call.enqueue(object : Callback<MovieResult> {
            override fun onResponse(call: Call<MovieResult>, response: Response<MovieResult>) {
                ...
                if (result?.results != null) {
                    val movies = result.results as ArrayList<Movie>
                    mutableUpcomingMovies.value = movies
                }
            }
